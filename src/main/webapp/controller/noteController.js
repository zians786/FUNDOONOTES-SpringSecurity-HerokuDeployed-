...
todo.controller('noteController',function($mdSidenav,toastr,$scope,noteService,loginService,labelService,$location,$mdDialog,$mdToast,$interval,$filter){
	...
	$scope.delete=function(note){
		console.log(note);
		var noteDelete=noteService.delete(note.noteId);
	...
	$scope.trash=function(note){
		...
		var trashNote=noteService.trash(note.noteId,note.inTrash);
	...
	$scope.restore=function(note){
		console.log(note);
		var trashNote=noteService.trash(note.noteId);
		trashNote.then(function(response){
	...
	$scope.archive=function(note){
		...
		var archiveNote=noteService.archive(note.noteId,note.archived);
	...
	// for pin
	    $scope.pin=function(note){
	    	...
	    	console.log(note);
			var pinResp=noteService.pin(note.noteId,note.pinned);
...
//		        		
	...
	// for color-picker
	    ...
	    $scope.colorChanged = function(newColor,note) {
	    	note.color=newColor;
	    	var colorResponse=noteService.color(note.noteId,newColor.substring(1));
